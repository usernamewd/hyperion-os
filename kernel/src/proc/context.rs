...
//!
//! We save the AArch64 *callee-saved* registers (`x19..x30`, `sp`, plus a
//! couple of spares) into a [`Context`] and restore the next thread's. The
//! actual switch is a tiny piece of assembly so we have full control over
//! register layout.
//!
...
///
/// We arrange this by:
///   * setting `sp` to `stack_top`
///   * setting `lr` to a small assembly trampoline that loads `entry` and
///     `arg` from callee-saved registers and branches to it
///   * stashing `entry` in `x19` and `arg` in `x20` so the trampoline can
///     pick them up.
pub fn init_context(
    ctx: &mut Context,
