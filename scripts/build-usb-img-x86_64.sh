#!/usr/bin/env bash
#
# Build a raw GPT-partitioned x86_64 disk image of Hyperion that is
# directly flashable to a USB stick (Rufus "DD mode", `dd`,
# balenaEtcher).
#
# Layout (mirror of the aarch64 build-usb-img.sh):
#
#   LBA 0      : protective MBR
#   LBA 1+2    : GPT primary header + entries
#   2 MiB ..   : EFI System Partition (FAT32, type EF00)
#                contains /EFI/BOOT/BOOTX64.EFI (Hyperion EFI stub)
#   tail       : GPT secondary header + entries
#
# Unlike the ISO, this is a real disk image — UEFI x86_64 firmware
# enumerates the GPT, finds the ESP by partition type, and runs the
# Hyperion EFI stub directly. There is no GRUB in the chain.
#
# Usage:
#   scripts/build-usb-img-x86_64.sh [out.img]
#
# Requirements:
#   sgdisk (gdisk pkg), mtools, dosfstools, cargo,
#   the x86_64-unknown-uefi rustup target.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

OUT="${1:-target/hyperion-x86_64-usb.img}"
...
STAGE_FAT="target/usb-stage-x86_64/esp.img"
SECTOR=512
...
for tool in sgdisk mmd mcopy mkfs.vfat cargo; do
    if ! command -v "$tool" >/dev/null 2>&1; then
        echo "build-usb-img-x86_64: missing required tool: $tool" >&2
        exit 1
    fi
done

echo "==> building x86_64 kernel payload"
...
if [[ ! -f "$EFI_BIN" ]]; then
    echo "build-usb-img-x86_64: EFI stub not found at $EFI_BIN" >&2
    exit 1
fi
...
mcopy -i "$STAGE_FAT" "$EFI_BIN" "::EFI/BOOT/BOOTX64.EFI"
